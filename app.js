...
(function () {
  ...
  const ART = {
    ...
    prime: '#d63b3b',                       // border of any box holding a prime
    serif: "'Frank Ruhl Libre','Cardo','Times New Roman',serif",
  };

  /* prime numbers are flagged wherever a box holds one: a red border on the
   * milui subtotal and total boxes and on the number-insight cards */
  const isPrime = (n) => { const a = G.analyzeNumber(n); return !!(a && a.isPrime); };
  const primeEdge = (n) => (isPrime(n) ? { stroke: ART.prime, 'stroke-width': 2.5 } : {});
  ...
  const CIPHER_HUES = {
    ...
    en: { sumerian: 237, ordinal: 200, reverse: 188, reduction: 174,
          isopsephy: 152, building: 218, buildingSumerian: 259, ladder: 330, ladderSumerian: 305 },
  };
  ...
  function renderMilui(analysis) {
    ...
    sg.appendChild(el('rect', {
      ...
      fill: ART.sumFill, stroke: isPrime(total) ? ART.prime : ART.sumEdge,
      'stroke-width': isPrime(total) ? 4 : 2.5,
    }));
  ...
  function drawMiluiBlock(g, cx, yTop, letter, spelling, S, SP) {
    ...
    const sub = G.spellingValue(spelling);
    g.appendChild(el('rect', {
      x: cx - sw / 2, y: subTop, width: sw, height: sh, rx: 8,
      fill: ART.subtotalFill, ...primeEdge(sub),
    }));
    g.appendChild(txt(cx, subTop + sh / 2, String(sub),
  ...
  function drawMiluiBlock2(g, cx, yTop, letter, spelling, S, maxK) {
    ...
    const sw = S, sh = S * 0.5;
    const sub = G.miluiDmiluiValue(letter, state.scheme);
    g.appendChild(el('rect', {
      x: cx - sw / 2, y: subTop, width: sw, height: sh, rx: 8,
      fill: ART.subtotalFill, ...primeEdge(sub),
    }));
    g.appendChild(txt(cx, subTop + sh / 2, String(sub), S * 0.26, ART.subtotalText, 700));
  }
  ...
  function factBlock(label, value) {
    ...
    d.className = 'fact-block' + (m.prime ? ' prime' : '');
  ...
  // session-only note from the comment box; appended to the text report,
  // never persisted anywhere
  function commentLines(rule) {
    const note = $('comment').value.trim();
    if (!note) return [];
    const rows = note.split(/\r?\n/);
    return [rule, `Comment: ${rows[0]}`, ...rows.slice(1)];
  }
  ...
  function buildShortLine(analysis) {
    ...
    if (state.view === 'milui') {
      ...
      for (const word of analysis.words.filter((w) => w.script === 'he')) {
        ...
        for (const letter of word.kept) {
          ...
          const sp = G.chooseSpelling(key, state.scheme);
          if (!sp) continue;
          total += deep ? G.miluiDmiluiValue(key, state.scheme) : G.spellingValue(sp);
          spelt.push(sp);
          // second filling: every letter of the first spelling spelled again
          if (deep) spelt2.push(Array.from(sp)
            .map((c) => G.chooseSpelling(c, state.scheme)).join(' '));
        }
    ...
    const cipherKey = state.cipher[analysis.script] || G.DEFAULT_CIPHER[analysis.script];
    const spec = G.CIPHERS[analysis.script][cipherKey];
    /* each letter(value) token is wrapped in an LTR isolate (U+2066…U+2069)
     * so the bidi algorithm can't pull the parens and plus signs out of
     * order when Hebrew letters and digits interleave */
    const tok = (letter, value) => `⁦${letter}|${value}|⁩`;
    const name = spec.line || spec.label;
    const chain = (word) => {
      if (word.folded) {
        return word.folded.kept.map((l, i) =>
          tok(G.stripMarks(l), word.folded.values[i])).join(' + ');
      }
      const labels = word.subs || word.groups;
      return word.kept.map((l, i) =>
        tok(labels ? labels[i] : G.stripMarks(l), word.values[i])).join(' + ');
    };
    const detail = (word) => {
      const sum = word.values.reduce((a, b) => a + b, 0);
      // building ciphers: the prefix steps are the derivation (י יה יהו יהוה)
      if (word.building) return `${word.groups.join(' ')} = ${sum}`;
  ...
  const PANEL = {
    ...
    gold: '#9c7827', indigo: '#4f5ed9', prime: '#d63b3b',
  ...
  function paintFactBlock(ctx, m, x, y, w, paint) {
    ...
    if (paint) {
      ...
      ctx.fill();
      ctx.strokeStyle = m.prime ? PANEL.prime : PANEL.line;
      ctx.lineWidth = m.prime ? 2 : 1;
      ctx.stroke();
    }
    let cy = y + pad;
  ...
  $('copy-line').addEventListener('click', async (e) => {
    ...
    const btn = e.currentTarget;
    try {
      await navigator.clipboard.writeText(buildShortLine(analysis));
      btn.textContent = 'Copied!';
    } catch {
      btn.textContent = 'Copy failed';
    }
